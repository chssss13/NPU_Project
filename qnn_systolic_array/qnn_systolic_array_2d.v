...
module qnn_systolic_array_2d (
    ...
    input wire [7:0] in_a0, input wire [7:0] in_a1,
    input wire [2:0] in_b0, input wire [2:0] in_b1,
    ...
    wire [7:0] a00_to_a01, a10_to_a11;
    wire [2:0] b00_to_b10, b01_to_b11;
    wire a00_to_a01_valid, a10_to_a11_valid;
    wire b00_to_b10_valid, b01_to_b11_valid;
    
    // 경계면 더미 와이어
    wire [7:0] d_a01, d_a11;
    wire [2:0] d_b10, d_b11;
    
    wire [7:0] pe00_out, pe01_out, pe10_out, pe11_out;

    // PE (0,0) - 무지연 다이렉트 셀
    qnn_pe_cell pe00 (
        .clk(clk), .rst_n(rst_n), .clr_accum(clr_accum), .relu_en(relu_en),
    ...
    qnn_pe_cell pe01 (
        ...
        .out_a(d_a01), .out_b(b01_to_b11), .out_a_valid(), .out_b_valid(b01_to_b11_valid), .out_data(pe01_out)
    );
    ...
    qnn_pe_cell pe10 (
        ...
        .out_a(a10_to_a11), .out_b(d_b10), .out_a_valid(a10_to_a11_valid), .out_b_valid(), .out_data(pe10_out)
    );
    ...
    qnn_pe_cell pe11 (
        ...
        .out_a(d_a11), .out_b(d_b11), .out_a_valid(), .out_b_valid(), .out_data(pe11_out)
    );
